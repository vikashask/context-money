import { db } from './db';
import { startOfMonth, endOfMonth, startOfWeek, endOfWeek, subMonths, format, isWeekend, eachDayOfInterval, parseISO } from 'date-fns';
...
export function generateInsights(expenses, monthlyBudget, monthlyIncome, prevMonthExpenses, categories) {
  const insights = [];
  ...
  const weekendAvg = weekendExpenses.reduce((s, e) => s + e.amount, 0) / weekendDays;
  const weekdayAvg = weekdayExpenses.reduce((s, e) => s + e.amount, 0) / weekdayDays;
  ...
  for (const [cat, amount] of Object.entries(catTotals)) {
    const catExpenses = expenses.filter((e) => e.category === cat && e.isRecurring);
...
export function getFinancialHealth(savingsRate, catTotals, total, monthlyTrend) {
  const anyCategoryOver40 = Object.values(catTotals).some((v) => total > 0 && (v / total) * 100 > 40);
  const increasing3Months = monthlyTrend.length >= 3 &&
    monthlyTrend.slice(-3).every((v, i, arr) => i === 0 || v > arr[i - 1]);

  if (savingsRate > 30 && !anyCategoryOver40 && !increasing3Months) {
    return { level: 'healthy', color: '#22c55e', emoji: '🟢', text: 'Healthy — great savings rate and balanced spending' };
  }
  if (savingsRate < 15 || increasing3Months) {
    return { level: 'alert', color: '#ef4444', emoji: '🔴', text: 'Alert — savings rate is low or spending is climbing' };
  }
  return { level: 'watch', color: '#f59e0b', emoji: '🟡', text: 'Watch — room for improvement in savings or category balance' };
}
