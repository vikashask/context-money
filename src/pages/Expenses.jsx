...
export default function Expenses() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [allExpenses, search, selectedCategories, dateFrom, dateTo, sortBy]);

  const filteredTotal = filtered.reduce((s, e) => s + e.amount, 0);
  const hasActiveFilters = search || selectedCategories.length > 0 || dateFrom || dateTo;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-heading font-bold text-navy dark:text-white">Expenses</h1>
          <p className="text-xs text-gray-400 dark:text-dark-muted mt-0.5">
            {filtered.length} entries · Total: <span className="font-semibold font-mono-amount">{formatAmount(filteredTotal, sym)}</span>
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { setShowAdd(true); if (categories?.length > 0 && !addCategory) setAddCategory(categories[0].name); }}
      ...
      {showFilters && (
        <div className="bg-white dark:bg-dark-card rounded-2xl p-4 border border-gray-100 dark:border-dark-border space-y-3 animate-fade-in-up">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-gray-500 dark:text-dark-muted">Filters</span>
            {hasActiveFilters && (
              <button onClick={clearFilters} className="text-[10px] text-coral hover:text-coral-light">Clear all</button>
            )}
          </div>
          <input
            type="text"
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="px-2 py-1.5 rounded-lg border border-gray-200 dark:border-dark-border bg-cream dark:bg-dark text-xs text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
            <span className="text-gray-400 text-xs">to</span>
            <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="px-2 py-1.5 rounded-lg border border-gray-200 dark:border-dark-border bg-cream dark:bg-dark text-xs text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-2 py-1.5 rounded-lg border border-gray-200 dark:border-dark-border bg-cream dark:bg-dark text-xs text-navy dark:text-dark-text ml-auto focus:outline-none focus:ring-2 focus:ring-coral/40">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 bg-coral/10 border border-coral/30 rounded-xl px-4 py-2.5 animate-fade-in-up">
          <span className="text-sm text-coral font-medium">{selectedIds.size} selected</span>
          <button onClick={selectAll} className="text-xs text-gray-500 dark:text-dark-muted hover:text-navy dark:hover:text-white">
            {selectedIds.size === filtered.length ? 'Deselect all' : 'Select all'}
          </button>
          <button onClick={deleteSelected} className="ml-auto text-sm text-red-600 font-medium hover:text-red-700">
            🗑️ Delete
          </button>
          <button onClick={() => setSelectedIds(new Set())} className="text-sm text-gray-500">
            ✕
          </button>
        </div>
      )}
      ...
      {groupedByDate.length > 0 ? (
        <div className="space-y-3">
          {groupedByDate.map(([dateKey, dateExpenses]) => {
            ...
            return (
              <div key={dateKey} className="bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border overflow-hidden">
                {/* Date header */}
                <div className="flex items-center justify-between px-4 py-2 bg-gray-50 dark:bg-dark-border/50 border-b border-gray-100 dark:border-dark-border">
                  <span className="text-xs font-medium text-gray-500 dark:text-dark-muted">
                    {format(new Date(dateKey + 'T00:00:00'), 'EEEE, dd MMM yyyy')}
                  </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {expense.note && <p className="text-xs text-gray-400 dark:text-dark-muted truncate">{expense.note}</p>}
                        </div>
                        ...
                        {isConfirmingDelete ? (
                          <div className="flex gap-1">
                            <button onClick={() => deleteSingle(expense.id)} className="px-2 py-1 bg-red-500 text-white rounded-lg text-[10px] font-medium">Del</button>
                            <button onClick={() => setConfirmDeleteId(null)} className="px-2 py-1 bg-gray-200 dark:bg-dark-border text-gray-600 rounded-lg text-[10px]">No</button>
                          </div>
                        ) : (
        ...
        <div className="bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border text-center py-12 text-gray-400 dark:text-dark-muted">
          ...
          <p className="text-sm font-medium mb-1">{hasActiveFilters ? 'No expenses match your filters' : 'No expenses yet'}</p>
          {hasActiveFilters ? (
            <button onClick={clearFilters} className="mt-2 text-xs text-coral hover:text-coral-light">Clear filters</button>
          ) : (
            <button
              onClick={() => { setShowAdd(true); if (categories?.length > 0 && !addCategory) setAddCategory(categories[0].name); }}
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white dark:bg-dark-card rounded-t-2xl md:rounded-2xl p-5 w-full md:max-w-md border-t md:border border-gray-200 dark:border-dark-border animate-fade-in-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowAdd(false)} className="p-1 text-gray-400 hover:text-gray-600" data-modal-close>✕</button>
            </div>
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-gray-500 dark:text-dark-muted mb-1">Note</label>
                  <input type="text" value={addNote} onChange={(e) => setAddNote(e.target.value)} placeholder="Optional" className="w-full px-3 py-2.5 rounded-xl bg-cream dark:bg-dark border border-gray-200 dark:border-dark-border text-sm text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 dark:text-dark-muted mb-1">Date</label>
                  <input type="date" value={addDate} onChange={(e) => setAddDate(e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-cream dark:bg-dark border border-gray-200 dark:border-dark-border text-sm text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
                </div>
              </div>

              <label className="flex items-center gap-2 text-sm text-navy dark:text-dark-text">
                <input type="checkbox" checked={addRecurring} onChange={(e) => setAddRecurring(e.target.checked)} className="accent-coral" />
                Recurring expense
              </label>
      ...
      {editingExpense && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setEditingExpense(null)}>
          <div className="bg-white dark:bg-dark-card rounded-t-2xl md:rounded-2xl p-5 w-full md:max-w-md border-t md:border border-gray-200 dark:border-dark-border animate-fade-in-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-heading font-bold text-navy dark:text-white">Edit Expense</h3>
              <button onClick={() => setEditingExpense(null)} className="p-1 text-gray-400 hover:text-gray-600" data-modal-close>✕</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-500 dark:text-dark-muted mb-1">Amount</label>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {categories?.map((c) => (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setEditingExpense({ ...editingExpense, category: c.name })}
                      className={`flex flex-col items-center gap-0.5 p-2 rounded-xl text-center transition-all ${
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-gray-500 dark:text-dark-muted mb-1">Note</label>
                  <input type="text" value={editingExpense.note || ''} onChange={(e) => setEditingExpense({ ...editingExpense, note: e.target.value })} className="w-full px-3 py-2.5 rounded-xl bg-cream dark:bg-dark border border-gray-200 dark:border-dark-border text-sm text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 dark:text-dark-muted mb-1">Date</label>
                  <input type="date" value={editingExpense.date} onChange={(e) => setEditingExpense({ ...editingExpense, date: e.target.value })} className="w-full px-3 py-2.5 rounded-xl bg-cream dark:bg-dark border border-gray-200 dark:border-dark-border text-sm text-navy dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-coral/40" />
                </div>
              </div>

              <label className="flex items-center gap-2 text-sm text-navy dark:text-dark-text">
                <input type="checkbox" checked={editingExpense.isRecurring} onChange={(e) => setEditingExpense({ ...editingExpense, isRecurring: e.target.checked })} className="accent-coral" />
                Recurring expense
              </label>

              <div className="flex gap-2 pt-2">
                <button onClick={handleDeleteFromEdit} className="px-4 py-2.5 border border-red-200 dark:border-red-800 text-red-600 rounded-xl text-sm font-medium hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
                  🗑️ Delete
                </button>
                <button onClick={handleEdit} className="flex-1 py-2.5 bg-coral hover:bg-coral-light text-white rounded-xl text-sm font-medium transition-colors">
                  Save Changes
                </button>
              </div>
            </div>
